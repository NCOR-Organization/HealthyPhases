...
with open("src/phases/ontologies/phases.owl", "r") as f:
    ontology = f.read()
...
memory:
    ...
    def new():
        ...
        # Resolve the chat model through the model registry so that provider
        # credentials (e.g. OPENAI_API_KEY) come from the provider module's
        # factory rather than ambient env vars. ELO is pinned to gpt-5
        # explicitly — change the canonical id below to swap models.
        chat_model = module.engine.services.model_registry.get_chat_model("gpt-5").model
