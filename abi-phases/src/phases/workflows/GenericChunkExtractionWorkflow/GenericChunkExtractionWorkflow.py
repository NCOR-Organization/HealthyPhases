...
SELECT ?chunk ?chunk_id ?text ?path WHERE {{
    ...
    # Matching by property (not URI) so historical Extractions persisted under
    # different URI schemes still count.
    FILTER NOT EXISTS {{
        GRAPH <{EXTRACTIONS_GRAPH}> {{
            ?prior_extraction phases-doc:prompt_hash {prompt_hash_lit} ;
                              phases-doc:model_name {model_name_lit} .
            ?prior_item phases-doc:extracted_by ?prior_extraction ;
