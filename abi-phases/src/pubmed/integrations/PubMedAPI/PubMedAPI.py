...
from pubmed.ontologies.PubMed import (
    Journal,
    JournalIssue,
    PubMedPaperSummary,
)
...
class PubMedIntegration(Integration):
        ...
        *,
        ...
        try:
            if method.upper() == "POST":
                ...
                response = requests.get(
                    url, params=query_params, timeout=self.__configuration.timeout
                )
            response.raise_for_status()
        except requests.exceptions.RequestException as exc:
            logger.error(f"PubMed API request failed: {exc}")
            raise IntegrationConnectionError(
                f"PubMed API request failed: {exc}"
            ) from exc
        ...
        if expect_json:
            try:
                ...
                logger.error(
                    f"PubMed API returned invalid JSON response: {response.text}"
                )
                raise IntegrationConnectionError(
                    "PubMed API returned invalid JSON response"
                ) from exc
    ...
    def _build_summary(self, pmid: str, doc: dict | None) -> PubMedPaperSummary:
        ...
        summary = PubMedPaperSummary(
            ...
            journalTitleLiteral=_clean_optional_str(
                (doc.get("fulljournalname") or doc.get("source")) if doc else None
            ),
            publicationDate=_parse_publication_date(
                doc.get("pubdate") if doc else None
            ),
            sortPublicationDate=_parse_sort_pubdate(
                doc.get("sortpubdate") if doc else None
            ),
            authorLiteral="; ".join(
                author.get("name")
        ...
        issn_value = _clean_optional_str(
            (doc.get("issn") or doc.get("essn")) if doc else None
        )
    ...
    def _summaries(self, id_list: list[str]) -> list[PubMedPaperSummary]:
        ...
        if not id_list:
            return []
        new_ids = [
            _id for _id in id_list if not cache.exists(f"pubmed_paper_summary_{_id}")
        ]
            ...
            for _id in chunk_ids:
                try:
                    ...
                    if summary_index < len(chunk_summaries):
                        cache.set_pickle(
                            f"pubmed_paper_summary_{_id}",
                            chunk_summaries[summary_index],
                        )
                except ValueError:
    ...
    def search_date_range(
        ...
        *,
        ...
        start_dt_opt = _parse_publication_date(start_date) or _parse_publication_date(
            start_date.replace("-", " ")
        )
        ...
        while windows:
            ...
            count = self._count_for_range(
                query, mindate=mindate_str, maxdate=maxdate_str, sort=sort
            )
            if count == 0:
                continue
