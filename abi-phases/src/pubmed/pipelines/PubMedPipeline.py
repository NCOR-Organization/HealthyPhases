...
from pubmed.integrations.PubMedAPI import (
...
class PubMedPipelineParameters(PipelineParameters):
    ...
    query: Annotated[str, Field(description="The query to search for in PubMed")]
    start_date: Annotated[
        str, Field(description="The start date to search for in PubMed")
    ]
    end_date: (
        Annotated[str, Field(description="The end date to search for in PubMed")] | None
    ) = None
    sort: (
        Annotated[
    ...
    max_results: (
        Annotated[
            int,
            Field(ge=1, le=10000, description="Maximum number of results to return"),
        ]
        | None
    ) = 100
