...
@dg.job(
    ...
    description=(
        "Ingest the solitude papers, then run every GenericChunkExtractionWorkflow "
        "pipeline (causes / how / when / where / effects) over the resulting chunks."
    ),
)
def ingest_solitude_papers_job() -> None:
    run_solitude_extractions_op(start=ingest_solitude_papers_op())
    ...
    def New(cls) -> "PhasesOrchestration":
        return cls(
            definitions=dg.Definitions(
                assets=[],
                schedules=[],
                jobs=[ingest_solitude_papers_job, normalize_paper_paths_job],
                sensors=[],
            )
        )
